...
sequenceDiagram
    autonumber
    actor User as Developer (Hinglish)
    ...
    note over User, AWS: === SCENARIO 1: INITIALIZATION ===
    User->>VSCode: Open Dev-Sarathi
    VSCode->>AWS: Authenticate (IAM Role)
    AWS-->>VSCode: Session Active

    note over User, AWS: === SCENARIO 2: VERNACULAR LEARNING (Gyan-Setu) ===
    User->>VSCode: "Bhai, S3 storage kya hota hai?"
    ...
    Orch-->>User: "S3 ek digital godown ki tarah hai..." (Vernacular)

    note over User, AWS: === SCENARIO 3: CODE GENERATION (Vani-Srijan) ===
    User->>VSCode: "Python mein Fibonacci series ka function likho"
    VSCode->>Orch: Intent: CODE_GEN (Python)
    Orch->>Orch: Generate Code Snippet
    Orch-->>User: Returns Python Code Block
    User->>VSCode: "Insert Code" (Click)

    note over User, AWS: === SCENARIO 4: LOCAL AUTOMATION (Vani-Srijan) ===
    User->>VSCode: "Code change commit kardo"
    VSCode->>Orch: Intent: GIT_COMMIT
    Orch->>Guard: Safety Check
    Guard-->>Orch: Safe (Allow)
    Orch->>Local: git commit -m "Update logic"
    Local-->>User: "Commit Successful: [a1b2c3]"

    note over User, AWS: === SCENARIO 5: SAFETY INTERCEPTION (Karma-Kavach) ===
    User->>VSCode: "Production DB uda do" (Delete DB)
    VSCode->>Orch: Intent: DELETE_RDS
    Orch->>Guard: Evaluate Risk
    Guard-->>Orch: BLOCK (High Risk)
    Orch-->>User: "Warning: Destructive Action hoga! Override?"
    User->>Orch: "Cancel" (Action Aborted)

    note over User, AWS: === SCENARIO 6: ERROR DIAGNOSTICS (Dosh-Drishti) ===
    User->>VSCode: "Naya EC2 launch karo"
    ...
    Drishti-->>User: "Error: Capacity nahi hai. Aap  'ap-south-1a' try kar sakte hain."
